...
export async function POST(request: NextRequest) {
  try {
    ...
    console.log("[v0] Translation API called:", { text, fromLanguage, toLanguage })

    if (!text || !fromLanguage || !toLanguage) {
      console.log("[v0] Translation API: Missing required fields")
      return NextResponse.json({ error: "Missing required fields: text, fromLanguage, toLanguage" }, { status: 400 })
    }

    // If source and target languages are the same, return original text
    if (fromLanguage === toLanguage) {
      console.log("[v0] Translation API: Same language, returning original")
      return NextResponse.json({ translatedText: text })
    }
    ...
    console.log("[v0] Translation API: Calling Gemini for translation")
    const { text: translatedText } = await generateText({
...
Translation:`,
    ...
    console.log("[v0] Translation API: Success:", translatedText.trim())
    return NextResponse.json({
      translatedText: translatedText.trim(),
      fromLanguage,
      toLanguage,
