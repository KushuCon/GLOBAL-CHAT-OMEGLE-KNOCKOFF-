...
export default function SocketChatInterface({ roomID, username, language, onLeave }: SocketChatInterfaceProps) {
  ...
  useEffect(() => {
    ...
    const receiveTranslationHandler = (data: {
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.message === data.originalMessage && msg.username === data.sender
            ? {
                ...msg,
                translatedMessage: data.translatedMessage,
                originalLanguage: data.fromLanguage,
                translatedLanguage: data.toLanguage,
              }
            : msg,
        ),
      )

      if (data.fromLanguage && data.fromLanguage !== language) {
        setPartnerLanguage(data.fromLanguage)
      }
    }
    ...
    const ownTranslatedMessageHandler = (data: {
      ...
      setMessages((prev) =>
        prev.map((msg) =>
          msg.isOwn && msg.message === data.originalMessage
            ? {
                ...msg,
                translatedMessage: data.translatedMessage,
    ...
    socket.on("receiveMessage", receiveMessageHandler)
    socket.on("receiveTranslation", receiveTranslationHandler)
    socket.on("ownTranslatedMessage", ownTranslatedMessageHandler)

    return () => {
      socket.off("receiveMessage", receiveMessageHandler)
      socket.off("receiveTranslation", receiveTranslationHandler)
      socket.off("ownTranslatedMessage", ownTranslatedMessageHandler)
    }
  }, [socket, roomID, language])
